...
CHIP_ERROR ThreadStackManagerImpl::_InitThreadStack()
{
    ...
    uint64_t nodeId;
    /* SONAMING
     * libopenthread-cli.so.[version]
     * ex) libopenthread-cli.so.1.7.4
     */
    snprintf(fileName, sizeof(fileName), "/usr/lib/libopenthread-cli.so.%s", IoTer::getSamsungThreadVersion());
    ...
    snprintf(threadSettingsFileName, sizeof(fileName), "0_%lx.data", nodeId);

    ChipLogProgress(DeviceLayer, "Thread stack manager initialized..gthreadId[%ld], threadSettingsFileName[%s]", threadId,
                    threadSettingsFileName);
...
{
    ChipLogProgress(DeviceLayer, "TODO: _LockThreadStack");
}
...
{
    ChipLogProgress(DeviceLayer, "TODO: _UnlockThreadStack");
}
