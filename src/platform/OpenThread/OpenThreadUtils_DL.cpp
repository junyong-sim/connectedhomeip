...
bool FormatOpenThreadError(char * buf, uint16_t bufSize, CHIP_ERROR err)
{
    ...
    const char * desc = OpenThreadDL_Manager::getInstance().mOtThreadErrorToString(otErr);
...
void LogOpenThreadStateChange(otInstance * otInst, uint32_t flags)
{
    ...
    {
        ChipLogDetail(DeviceLayer, "   Device Role: %s",
                      OpenThreadRoleToStr(OpenThreadDL_Manager::getInstance().mOtThreadGetDeviceRole(otInst)));
    }
    if ((flags & kParamsChanged) != 0)
    {
        ChipLogDetail(DeviceLayer, "   Network Name: %s", OpenThreadDL_Manager::getInstance().mOtThreadGetNetworkName(otInst));
        ChipLogDetail(DeviceLayer, "   PAN Id: 0x%04X", OpenThreadDL_Manager::getInstance().mOtLinkGetPanId(otInst));
        {
            const otExtendedPanId * exPanId = OpenThreadDL_Manager::getInstance().mOtThreadGetExtendedPanId(otInst);
        ...
        ChipLogDetail(DeviceLayer, "   Channel: %d", OpenThreadDL_Manager::getInstance().mOtLinkGetChannel(otInst));
        {
            const otMeshLocalPrefix * otMeshPrefix = OpenThreadDL_Manager::getInstance().mOtThreadGetMeshLocalPrefix(otInst);
        ...
        {
#if OPENTHREAD_API_VERSION >= 126
            const otNetworkKey * otKey = OpenThreadDL_Manager::getInstance().mOtThreadGetNetworkKey(otInst);
    ...
    {
        ChipLogDetail(DeviceLayer, "   Partition Id: 0x%" PRIX32,
                      OpenThreadDL_Manager::getInstance().mOtThreadGetPartitionId(otInst));
    }
    if ((flags & (OT_CHANGED_IP6_ADDRESS_ADDED | OT_CHANGED_IP6_ADDRESS_REMOVED)) != 0)
    {
        ChipLogDetail(DeviceLayer, "   Thread Unicast Addresses:");
        for (const otNetifAddress * addr = OpenThreadDL_Manager::getInstance().mOtIp6GetUnicastAddresses(otInst); addr != NULL;
             addr                        = addr->mNext)
        {
...
void LogOpenThreadPacket(const char * titleStr, otMessage * pkt)
{
    ...
    pktLen = OpenThreadDL_Manager::getInstance().mOtMessageGetLength(pkt);

    if (pktLen >= sizeof(headerData))
    {
        OpenThreadDL_Manager::getInstance().mOtMessageRead(pkt, 0, headerData, sizeof(headerData));
