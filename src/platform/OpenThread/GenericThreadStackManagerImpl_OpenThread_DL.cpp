...
bool GenericThreadStackManagerImpl_OpenThread<ImplClass>::_HaveRouteToAddress(const Inet::IPAddress & destAddr)
{
    ...
    if (IsThreadAttachedNoLock())
    {
        ...
        // advertise those routes specifically.
        {
            ...
            while ((otErr = OpenThreadDL_Manager::getInstance().mOtNetDataGetNextRoute(Impl()->OTInstance(), &routeIter, &routeConfig)) == OT_ERROR_NONE)
            {
...
void GenericThreadStackManagerImpl_OpenThread<ImplClass>::_OnPlatformEvent(const ChipDeviceEvent * event)
{
    if (event->Type == DeviceEventType::kThreadStateChange)
    {
        ...
        {
            const otSrpClientHostInfo * hostInfo = OpenThreadDL_Manager::getInstance().mOtSrpClientGetHostInfo(Impl()->OTInstance());
        ...
        ChipLogProgress(DeviceLayer, "GenericThreadStackManagerImpl_OpenThread::_OnPlatformEvent : old [%d], new [%d], RoleChanged[%d]", mIsAttached, isThreadAttached, event->ThreadStateChange.RoleChanged);
...
bool GenericThreadStackManagerImpl_OpenThread<ImplClass>::_IsThreadAttached(void)
{
    ...
    Impl()->LockThreadStack();
    curRole = OpenThreadDL_Manager::getInstance().mOtThreadGetDeviceRole(mOTInst);
    ChipLogProgress(DeviceLayer, "GenericThreadStackManagerImpl_OpenThread::_IsThreadAttached role [%d]", curRole);
    Impl()->UnlockThreadStack();
...
{
    ChipLogProgress(DeviceLayer, "GenericThreadStackManagerImpl_OpenThread::_AttachToThreadNetwork");
...
#if CHIP_SAMSUNG_UI_LOGGING
        IoTer::pipe_logging("step:60:Thread-Onboarding is completed", IoTer::getSamsungDeviceNumber());
#endif
    }
}
...
GenericThreadStackManagerImpl_OpenThread<ImplClass>::_StartThreadScan(NetworkCommissioning::ThreadDriver::ScanCallback * callback)
{
    ...
    error = MapOpenThreadError(OpenThreadDL_Manager::getInstance().mOtThreadDiscover(mOTInst, 0,                       /* all channels */
...
bool GenericThreadStackManagerImpl_OpenThread<ImplClass>::_HaveMeshConnectivity(void)
{
    ...
    else
    {
        ...
        while (OpenThreadDL_Manager::getInstance().mOtThreadGetNextNeighborInfo(mOTInst, &neighborIter, &neighborInfo) == OT_ERROR_NONE)
        {
    ...
    AttributeId attributeId, app::AttributeValueEncoder & encoder)
{
    ...
    case ThreadNetworkDiagnostics::Attributes::NeighborTable::Id: {
        err = encoder.EncodeList([this](const auto & aEncoder) -> CHIP_ERROR {
            ...
            while (OpenThreadDL_Manager::getInstance().mOtThreadGetNextNeighborInfo(mOTInst, &iterator, &neighInfo) == OT_ERROR_NONE)
            {
    ...
    case ThreadNetworkDiagnostics::Attributes::BetterPartitionAttachAttemptCount::Id: {
        uint16_t betterPartitionAttachAttempts = OpenThreadDL_Manager::getInstance().mOtThreadGetMleCounters(mOTInst)->mBetterPartitionAttachAttempts;
    ...
    case ThreadNetworkDiagnostics::Attributes::TxIndirectMaxRetryExpiryCount::Id: {
        uint32_t txIndirectMaxRetryExpiry = OpenThreadDL_Manager::getInstance().mOtLinkGetCounters(mOTInst)->mTxIndirectMaxRetryExpiry;
...
#if CHIP_DEVICE_CONFIG_THREAD_FTD
    Impl()->LockThreadStack();
    if (OpenThreadDL_Manager::getInstance().mOtThreadGetDeviceRole(DeviceLayer::ThreadStackMgrImpl().OTInstance()) != OT_DEVICE_ROLE_ROUTER)
    {
...
CHIP_ERROR GenericThreadStackManagerImpl_OpenThread<ImplClass>::DoInit(otInstance * otInst)
{
...
#if CHIP_DEVICE_CONFIG_THREAD_FTD
    Impl()->_SetThreadDeviceType(ConnectivityManager::kThreadDeviceType_FullEndDevice);
#elif CHIP_DEVICE_CONFIG_THREAD_SSED
    Impl()->_SetThreadDeviceType(ConnectivityManager::kThreadDeviceType_SynchronizedSleepyEndDevice);
#elif CHIP_DEVICE_CONFIG_ENABLE_SED
    Impl()->_SetThreadDeviceType(ConnectivityManager::kThreadDeviceType_SleepyEndDevice);
    ...
    SuccessOrExit(err);
#else
    Impl()->_SetThreadDeviceType(ConnectivityManager::kThreadDeviceType_MinimalEndDevice);
#endif
...
// * CSL period for SSED devices that listen for messages in scheduled time slots.
    ...
    uint32_t curIntervalMS = OpenThreadDL_Manager::getInstance().mOtLinkCslGetPeriod(mOTInst) * OT_US_PER_TEN_SYMBOLS / 1000;
#else
    uint32_t curIntervalMS = OpenThreadDL_Manager::getInstance().mOtLinkGetPollPeriod(mOTInst);
#endif
    otError otErr = OT_ERROR_NONE;
    if (interval.count() != curIntervalMS)
    {
        ...
        otErr         = OpenThreadDL_Manager::getInstance().mOtLinkCslSetPeriod(mOTInst, interval.count() * 1000 / OT_US_PER_TEN_SYMBOLS);
        curIntervalMS = OpenThreadDL_Manager::getInstance().mOtLinkGetPollPeriod(mOTInst) * OT_US_PER_TEN_SYMBOLS / 1000;
#else
        otErr         = OpenThreadDL_Manager::getInstance().mOtLinkSetPollPeriod(mOTInst, interval.count());
        curIntervalMS = OpenThreadDL_Manager::getInstance().mOtLinkGetPollPeriod(mOTInst);
#endif
...
CHIP_ERROR GenericThreadStackManagerImpl_OpenThread<ImplClass>::_JoinerStart(void)
{
    ...
    Impl()->LockThreadStack();
    VerifyOrExit(!OpenThreadDL_Manager::getInstance().mOtDatasetIsCommissioned(mOTInst) && OpenThreadDL_Manager::getInstance().mOtThreadGetDeviceRole(mOTInst) == OT_DEVICE_ROLE_DISABLED,
                 error = MapOpenThreadError(OT_ERROR_INVALID_STATE));
    VerifyOrExit(OpenThreadDL_Manager::getInstance().mOtJoinerGetState(mOTInst) == OT_JOINER_STATE_IDLE, error = MapOpenThreadError(OT_ERROR_BUSY));
...
void GenericThreadStackManagerImpl_OpenThread<ImplClass>::OnSrpClientStateChange(const otSockAddr * aServerSockAddr,
                                                                                 void * aContext)
{
    ChipLogProgress(DeviceLayer, "GenericThreadStackManagerImpl_OpenThread::OnSrpClientStateChange");
    ...
    if (aServerSockAddr)
    {
        ...
        otDnsQueryConfig dnsConfig = *OpenThreadDL_Manager::getInstance().mOtDnsClientGetDefaultConfig(ThreadStackMgrImpl().OTInstance());
                                                                               ...
                                                                               uint32_t aLeaseInterval, uint32_t aKeyLeaseInterval)
{
    ...
    for (typename SrpClient::Service & service : mSrpClient.mServices)
    {
        ...
        if (service.Matches(aInstanceName, aName))
        {
            ...
            SuccessOrExit(error = MapOpenThreadError(OpenThreadDL_Manager::getInstance().mOtSrpClientClearService(mOTInst, &service.mService)));
...
CHIP_ERROR GenericThreadStackManagerImpl_OpenThread<ImplClass>::ResolveAddress(intptr_t context, otDnsAddressCallback callback)
{
    ...
    CHIP_ERROR error = MapOpenThreadError(OpenThreadDL_Manager::getInstance().mOtDnsClientResolveAddress(ThreadStackMgrImpl().OTInstance(), fullHostName, callback,
                                                                            ...
                                                                            void * aContext)
{
    ...
    error = MapOpenThreadError(OpenThreadDL_Manager::getInstance().mOtDnsBrowseResponseGetServiceName(aResponse, type, sizeof(type)));

    VerifyOrExit(error == CHIP_NO_ERROR, );

    char serviceName[Dnssd::Common::kInstanceNameMaxLength + 1];
    while (OpenThreadDL_Manager::getInstance().mOtDnsBrowseResponseGetServiceInstance(aResponse, index, serviceName, sizeof(serviceName)) == OT_ERROR_NONE)
    {
        ...
        otError err = OpenThreadDL_Manager::getInstance().mOtDnsBrowseResponseGetServiceInfo(aResponse, serviceName, &serviceInfo);
                                                                           ...
                                                                           void * aContext)
{
    ...
    error = MapOpenThreadError(OpenThreadDL_Manager::getInstance().mOtDnsClientBrowse(mOTInst, fullServiceName, OnDnsBrowseResult, aContext, /* config */ nullptr));
                                                                                    ...
                                                                                    void * aContext)
{
    ...
    error = MapOpenThreadError(OpenThreadDL_Manager::getInstance().mOtDnsAddressResponseGetAddress(aResponse, 0, &address, nullptr));
    if (error == CHIP_NO_ERROR)
    {
                                                                             ...
                                                                             void * aContext)
{
    ...
    otErr = OpenThreadDL_Manager::getInstance().mOtDnsServiceResponseGetServiceInfo(aResponse, &serviceInfo);
