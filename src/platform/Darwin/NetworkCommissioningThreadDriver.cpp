...
#include <platform/Darwin/ThreadStackManagerImpl.h>
...
CHIP_ERROR DarwinThreadDriver::Init(BaseDriver::NetworkStatusChangeCallback * networkStatusChangeCallback)
{
...
void DarwinThreadDriver::Shutdown()
{
    ThreadStackMgrImpl().SetNetworkStatusChangeCallback(nullptr);
}

CHIP_ERROR DarwinThreadDriver::CommitConfiguration()
{
...
CHIP_ERROR DarwinThreadDriver::RevertConfiguration()
{
    mStagingNetwork = mSavedNetwork;
    return CHIP_NO_ERROR;
}

Status DarwinThreadDriver::AddOrUpdateNetwork(ByteSpan operationalDataset, MutableCharSpan & outDebugText, uint8_t & outNetworkIndex)
{
...
Status DarwinThreadDriver::RemoveNetwork(ByteSpan networkId, MutableCharSpan & outDebugText, uint8_t & outNetworkIndex)
{
    outDebugText.reduce_size(0);
    outNetworkIndex = 0;
    uint8_t extpanid[kSizeExtendedPanId];
...
Status DarwinThreadDriver::ReorderNetwork(ByteSpan networkId, uint8_t index, MutableCharSpan & outDebugText)
{
    outDebugText.reduce_size(0);
    uint8_t extpanid[kSizeExtendedPanId];
    if (!mStagingNetwork.IsCommissioned())
...
void DarwinThreadDriver::ConnectNetwork(ByteSpan networkId, ConnectCallback * callback)
{
#if CHIP_SAMSUNG_UI_LOGGING
    IoTer::pipe_logging("step:50:ConnectNetwork", IoTer::getSamsungDeviceNumber());
#endif
...
void DarwinThreadDriver::ScanNetworks(ThreadDriver::ScanCallback * callback)
{
#if CHIP_SAMSUNG_UI_LOGGING
    IoTer::pipe_logging("step:30:ScanNetworks", IoTer::getSamsungDeviceNumber());
#endif
...
size_t DarwinThreadDriver::ThreadNetworkIterator::Count()
{
    return driver->mStagingNetwork.IsCommissioned() ? 1 : 0;
}

bool DarwinThreadDriver::ThreadNetworkIterator::Next(Network & item)
{
